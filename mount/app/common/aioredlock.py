...
class Instance:
    ...
    async def _create_redis_pool(*args, **kwargs):
        ...
        if StrictVersion(aioredis.__version__) >= StrictVersion(
            "2.0.0"
        ):  # pragma no cover
            return await aioredis.Redis(
                host=kwargs.get("host"), port=kwargs.get("port")
            )
        elif StrictVersion(aioredis.__version__) >= StrictVersion(
            "1.0.0"
        ):  # pragma no cover
    ...
    async def connect(self):
        ...
        if isinstance(self.connection, Sentinel):
            ...
            redis_kwargs = kwargs
            redis_kwargs["host"] = address[0]
            redis_kwargs["port"] = address[1]
